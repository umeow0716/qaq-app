...
class GlobalProtectEspPacket {
  const GlobalProtectEspPacket({required this.sequence, required this.nextHeader, required this.payload});
...
class GlobalProtectEspCodec {
  ...
  factory GlobalProtectEspCodec.fromConfig(GlobalProtectIpsecConfig config) {
    ...
    if (config.encryptionAlgorithm != 'aes-128-cbc' && config.encryptionAlgorithm != 'aes128') {
      throw UnsupportedError('Unsupported GlobalProtect ESP encryption algorithm: ${config.encryptionAlgorithm}');
    }
  ...
  Uint8List encode(Uint8List payload, {required int nextHeader}) {
    ...
    final ciphertext = _cryptCbc(plaintext, _outboundEncryptionKey, iv, encrypt: true);
  ...
  GlobalProtectEspPacket? decode(Uint8List packet) {
    ...
    final plaintext = _cryptCbc(ciphertext, _inboundEncryptionKey, iv, encrypt: false);
  ...
  bool _acceptInboundSequence(int sequence) {
    ...
    if (sequence > _highestInboundSequence) {
      ...
      _inboundReplayBitmap = shift >= 64 ? 1 : ((_inboundReplayBitmap << shift) | 1) & 0xffffffffffffffff;
      _highestInboundSequence = sequence;
      return true;
    }
  ...
  static Uint8List _cryptCbc(Uint8List input, Uint8List key, Uint8List iv, {required bool encrypt}) {
    final cipher = CBCBlockCipher(AESEngine())..init(encrypt, ParametersWithIV<KeyParameter>(KeyParameter(key), iv));
